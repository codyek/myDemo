...
public class CheckAddress {
	 ...
	 */
	public String getAddressByUserID(String userid, String password,
			String timestamp) {
		try {
			// 判断主IP是否正常
			if (ConfigManager.masterIPState == 0) {
				// 主IP与可用IP不相等，则将可用IP设置为主IP
				if (!ConfigManager.masterIpAndPort
						.equals(ConfigManager.availableIpAndPort)) {
					// 将可用IP设置为主IP
			...
			// 主IP异常
				...
				if ((Calendar.getInstance().getTimeInMillis() - ConfigManager.LAST_CHECK_TIME) > ConfigManager.CHECK_TIME_INTERVAL) {
					// 检测主IP是否正常
					String address = checkMasterAddress(userid, password,
							timestamp);
				...
				if (ConfigManager.ipAndPortBak != null
						&& ConfigManager.ipAndPortBak.size() > 0) {
					for (int i = 0; i < ConfigManager.ipAndPortBak.size(); i++) {
						// 如果备IP与可用IP相等，则该备用IP可用
						if (ConfigManager.ipAndPortBak.get(i).equals(
								ConfigManager.availableIpAndPort)) {
							// 返回备用IP
				...
				String availableAddress = checkAddress(userid, password,
						timestamp);
		...
		try {
			...
			if (ipAndPortBakList != null && ipAndPortBakList.size() > 0) {
				...
				for (int i = 0; i < ipAndPortBakList.size(); i++) {
					...
					// 调用查询余额的方法检测连接是否可用
					result = checkAddressAvailable(userid, password, timestamp,
							ipAddressBak);
					// 查询余额成功
					if (result == 0) {
						...
						String domainBak = ConfigManager.ipAndDomainBakMap
								.get(ipAddressBak);
						...
						if (domainBak != null && !"".equals(domainBak)) {
							...
							if (ip != null && !"".equals(ip.trim())) {
								// 新获取的IP和端口
								String newIpAndPort = ip + ":"
										+ ipAddressBak.split(":")[1];
								// 调用查询余额的方法检测该备用IP对应的域名是否正常
								result = checkAddressAvailable(userid,
										password, timestamp, newIpAndPort);
								// 如果正常，则将该备用IP修改
								if (result == 0) {
									isDomainGet = true;
									// 移除旧的备用IP和域名的对应关系
									ConfigManager.ipAndDomainBakMap
											.remove(ipAddressBak);
									// 添加新的IP和域名的对应关系
									ConfigManager.ipAndDomainBakMap.put(
											newIpAndPort, domainBak);
									// 将新的IP和端口赋值给address
	 ...
	 */
	private int checkAddressAvailable(String userId, String password,
		...
		try {
			...
			// 通过HTTP POST调用，查询余额
			Message = executeNotKeepAlivePost(message, balanceHost
					+ ConfigManager.REQUEST_PATH + "get_balance");
	 ...
	 */
	private String executeNotKeepAlivePost(Object obj, String httpUrl)
			throws Exception {
	 ...
	 * @return
	 */
	private String checkMasterAddress(String userid, String password,
			String timestamp) {
		try {
			// 设置最近检查时间
			...
			// 调用查询余额的方法检测连接是否可用
			int result = checkAddressAvailable(userid, password, timestamp,
					ConfigManager.masterIpAndPort);
			...
			if (result == 0) {
				...
				System.out.println("主ipAddress["
						+ ConfigManager.masterIpAndPort + "]恢复正常。");
				return ConfigManager.availableIpAndPort;
			} else {
				// 如果主域名存在，则通过域名去获取IP。
				if (ConfigManager.masterDomainAndPort != null
						&& !"".equals(ConfigManager.masterDomainAndPort.trim())) {
					// 通过域名获取IP
					String ip = getIpByDomain(ConfigManager.masterDomainAndPort
							.split(":")[0]);
					...
					if (ip != null && !"".equals(ip.trim())) {
						...
						String newIpAndPort = ip
								+ ":"
								+ ConfigManager.masterDomainAndPort.split(":")[1];
						// 如果失败，通过新获取的IP检查余额
						result = checkAddressAvailable(userid, password,
								timestamp, newIpAndPort);
						...
						if (result == 0) {
							if (ConfigManager.masterIpAndPort
								...
								System.out.println("主ipAddress由["
										+ ConfigManager.masterIpAndPort
										+ "]切换为[" + newIpAndPort + "]。");
							}
							// 可用IP设置为新的主IP
							...
							System.out.println("通过域名获取的主IP正常,主IP和端口："
									+ newIpAndPort);
							return ConfigManager.availableIpAndPort;
						}
					}
