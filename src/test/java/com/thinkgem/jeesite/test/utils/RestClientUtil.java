...
public class RestClientUtil {
	...
	public final static Map<String, Object> EMPTY_BODY = new HashMap<String, Object>(
			1);
	 ...
	 *            返回的数据类型
	 * @param uriVariables
	 *            url自动匹配替换的参数，如url为api/{a}/{b},参数为["1","2"],则解析的url为api/1/2，
	 *            使用Map参数时，遵循按key匹配
	 * @return 结果对象
	 * @throws RestClientException
	 *             RestClient异常，包含状态码和非200的返回内容
	 */
	 ...
	 */
	public static <T> T exchange(String url, HttpMethod method,
			HttpHeaders headers, Class<T> responseType, Object... uriVariables)
			throws RestClientException {
	 ...
	 */
	public static <T> T exchange(String url, HttpMethod method,
			HttpHeaders httpHeaders, Object body, Class<T> responseType,
			Object... uriVariables) throws RestClientException {
		try {
			HttpEntity<Object> requestEntity = new HttpEntity(body, httpHeaders);
			requestEntity = convert(requestEntity);

			if (null != uriVariables && uriVariables.length == 1 && uriVariables[0] instanceof Map) {
				Map<String, ?> _uriVariables = (Map<String, ?>) uriVariables[0];
				return getClient().exchange(url, method, requestEntity,
						responseType, _uriVariables).getBody();
			}
	 ...
	 *            请求方式
	 * @param responseType
	 *            返回的数据类型，例：new ParameterizedTypeReference<List<Bean>>(){}
	 * @param uriVariables
	 *            url自动匹配替换的参数，如url为api/{a}/{b},参数为["1","2"],则解析的url为api/1/2，
	 *            使用Map参数时，遵循按key匹配
	 * @return 结果对象
	 ...
	 *            要提交的数据
	 * @param responseType
	 *            返回数据类型，例：new ParameterizedTypeReference<List<Bean>>(){}
	 *            返回bean时指定Class
	 * @param uriVariables
	 *            url自动匹配替换的参数，如url为api/{a}/{b},参数为["1","2"],则解析的url为api/1/2，
	 *            使用Map参数时，遵循按key匹配
	...
	*/
	 ...
	 * @return
	 */
	private static HttpEntity<Object> convert(HttpEntity<Object> requestEntity) {
        ...
        if (body instanceof Map) {
            ...
            requestEntity = new HttpEntity<Object>(multiValueMap, headers);
        }
        ...
        MultiValueMap<String, Object> formEntity = new LinkedMultiValueMap<String, Object>();
        ...
        return new HttpEntity<Object>(formEntity, headers);
    }
